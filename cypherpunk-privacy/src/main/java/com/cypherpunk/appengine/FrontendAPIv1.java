...
public class FrontendAPIv1 extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws IOException {
		...
		if (apiPath.equals("/hello")) { res.getWriter().println("hello"); }
			...
			if (appApiPath.startsWith("/versions")) {
				...
				catch (Exception e) { versionsFlavor = "default"; }
			...
			if (blogApiPath.equals("/index")) {
				...
				if (!forceUpdate) { setResponsePublicCacheHeaders(res, BLOGGER_API_CACHE_PERIOD); }
			...
			if (locationApiPath.equals("/world")) {
				...
				if (!forceUpdate) { setResponsePublicCacheHeaders(res, LOCATION_WORLD_CACHE_PERIOD); }
				res.getWriter().println(frontendJsonString);
			}
				...
				frontendJsonString = gson.toJson(cypherpunkResponse);
				if (!forceUpdate) { setResponsePublicCacheHeaders(res, LOCATION_LIST_CACHE_PERIOD); }
				res.getWriter().println(frontendJsonString);
			}
			else { res.sendError(404); }
		}
			...
			if (pricingApiPath.startsWith("/plans")) {
				...
				if (!forceUpdate) { setResponsePublicCacheHeaders(res, PRICING_PLANS_CACHE_PERIOD); }
				res.getWriter().println(frontendJsonString);
			}
			else { res.sendError(404); }
		}
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException {
		...
		// flag to force update of datastore/memcache
		boolean forceUpdate = false;
		if (req.getParameter("forceUpdate") != null) { forceUpdate = true; }
	...
	private void proxyRequestToCypherpunkBackend(HttpServletRequest req, HttpServletResponse res, HTTPMethod reqMethod, String cypherpunkURI, Class incomingRequestBean, Class outgoingRequestResponseBean) throws IOException {
		...
		if (queryString != null) { reqURI = cypherpunkURI + "?" + queryString; }
	...
	private void sendResponse(HttpServletResponse res, String str) throws IOException {
	...
	private URL buildZendeskURL(String zendeskURI) {
		...
		catch (MalformedURLException e) { zendeskURL = null; }
		return zendeskURL;
	}
	...
	private Map<String,Object> getCachedData(String apiURL, int secondsToMemcache, boolean useCache, boolean forceUpdate) {
		...
		if (useCache) { response = (String)mc.get(apiURL); }
		...
		if (responseData == null) {
			...
			try { response = requestDataAsString(HTTPMethod.GET, new URL(apiURL), null, null); }
	...
	private String requestZendeskData(HTTPMethod requestMethod, String zendeskURI, List<HTTPHeader> headers, String body) {
		...
		if (response != null) { zendeskResponse = getBodyFromResponse(response); }
	...
	private HTTPResponse requestData(HTTPMethod requestMethod, URL cypherpunkURL, List<HTTPHeader> headers, String body) {
		...
		try { response = requestDataFetch(request, headers, body); }
		catch (IOException e) {
	...
	private HTTPResponse requestDataFetch(HTTPRequest request, List<HTTPHeader> headers, String body) throws IOException, UnsupportedEncodingException {
